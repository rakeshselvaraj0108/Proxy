"""
Housing Domain - Full Data Collection Script
Downloads real content from MoHUA, state RERA authorities, the Model Tenancy Act,
NGDRS/property registration portals, and Wikipedia overviews, driven by
knowledge/housing/housing_source_registry.json.
"""
import json
import time
...
REGISTRY_PATH = KNOWLEDGE_ROOT / "housing_source_registry.json"
        ...
        if is_pdf:
            ...
            filepath = save(
                ...
                {"title": title, "authority": authority, "source_url": url,
                 "domain": "housing", "type": "pdf", "category": source.get("category")}
            )
        else:
...
def main():
    print("=" * 60)
    print("HOUSING DOMAIN — LIVE DATA DOWNLOAD")
    print("=" * 60)
