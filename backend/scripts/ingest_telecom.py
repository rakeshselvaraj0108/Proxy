"""
Telecom domain ingestion pipeline.
Reads all files under knowledge/telecom/, semantic-chunks them,
...
async def extract_kg(chunk: str, meta: dict, doc_id: str):
    ...
    if store.health_check().get("backend") != "neo4j":
        return
    prompt = f"""Extract telecom entities and relationships from this text.
Entities: Telecom Operator, Customer, SIM Card, Mobile Number, Broadband, Plan, VAS Service,
Complaint, Refund, TRAI Regulation, DoT Guideline, MNP, Network, Invoice, Consumer Right.

Text: {chunk[:1500]}

Return ONLY valid JSON:
{{"entities":[{{"label":"Telecom Operator","name":"Airtel"}}],
  "relationships":[{{"source":"Airtel","relation":"REGULATED_BY","target":"TRAI"}}]}}"""
    try:
...
async def run_pipeline(include_pdfs: bool = True):
    print(f"Starting Telecom Ingestion Pipeline â€” root: {KNOWLEDGE_ROOT}")
    docs = total_chunks = 0
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Ingest telecom knowledge into vector and graph stores.")
