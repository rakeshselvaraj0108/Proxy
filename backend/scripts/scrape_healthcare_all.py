"""
Healthcare Domain - Full Data Collection Script
Downloads real content from WHO, MoHFW, ICMR, National Health Mission,
MedlinePlus, CDC, NIH, AIIMS, National Health Authority, CDSCO, and
Wikipedia overviews, driven by
knowledge/healthcare/healthcare_source_registry.json.

This is a PUBLIC HEALTH EDUCATION corpus (disease/symptom info, preventive
care, vaccination schedules, clinical guidelines, lab reference info, drug
safety, patient rights, hospital quality standards) — not a dispute-
resolution corpus like the other domains. Content downloaded here is used
only for educational, evidence-based answers; it must never be presented as
...
REGISTRY_PATH = KNOWLEDGE_ROOT / "healthcare_source_registry.json"
        ...
        if is_pdf:
            ...
            filepath = save(
                ...
                {"title": title, "authority": authority, "source_url": url,
                 "domain": "healthcare", "type": "pdf", "category": source.get("category")}
            )
        else:
            ...
            filepath = save(
                ...
                {"title": title, "authority": authority, "source_url": url,
                 "domain": "healthcare", "type": "html_text",
...
def main():
    print("=" * 60)
    print("HEALTHCARE DOMAIN — LIVE DATA DOWNLOAD")
    print("=" * 60)
    ...
    for src in sources:
        ...
        if success:
            ...
            fail += 1
            failed_sources.append(f"{src['authority']} :: {src['title']} :: {src['url']}")
        time.sleep(1)
