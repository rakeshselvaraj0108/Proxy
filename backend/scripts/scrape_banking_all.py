"""
Banking Domain - Full Data Collection Script
Downloads real content from RBI (Master Directions, Ombudsman, notifications),
NPCI/UPI background, and Wikipedia overviews, driven by
knowledge/banking/banking_source_registry.json. Replaces the previous
fabricated placeholder dataset with genuinely downloaded documents.
"""
import json
import time
...
REGISTRY_PATH = KNOWLEDGE_ROOT / "banking_source_registry.json"
        ...
        if is_pdf:
            ...
            filepath = save(
                ...
                {"title": title, "authority": authority, "source_url": url,
                 "domain": "banking", "type": "pdf", "category": source.get("category")}
            )
        else:
...
def main():
    print("=" * 60)
    print("BANKING DOMAIN — LIVE DATA DOWNLOAD")
    print("=" * 60)
